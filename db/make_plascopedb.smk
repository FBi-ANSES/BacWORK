...
rule extract_plasmid_sequences:
	...
	run:
		shell("mkdir -p {params.outdir}/{params.genus}_{params.species}")
		shell("python scripts/grepSeq.py -p {input.PLSdb_fasta} -i \"{params.genus} {params.species}\" -o {output.fasta}")
		shell("sed -i 's/ .*$//g' {output.fasta}")
...
rule download_genomes:
	...
	shell:
		"""
		ncbi-genome-download --genera '{params.genus} {params.species}' --formats fasta --assembly-levels complete,chromosome --progress-bar bacteria 
